...
/**
 * @describe 用户组告警指标推送入口。
 * @date 2017-11-15
 * @author cym
 *
 */
public class UserGroupQuotaMain {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String fileName = Config.getInstance().getUserGroupQuotaPrefix() + "_" + start_time + ".csv";

			System.out.println("args is [" + start_time + "][" + end_time + "][" + path + "][" + fileName + "]");

			String read_sql = SqlUtils_ReadOracle.getReadUserGroupQuotafromOracle(start_time, end_time);

			List<StringBuffer> StringBufferList = ReadOracleDao.readUsergroupQuota(read_sql, start_time);
			...
			System.out.println("[" + start_time + "]用户组告警指标推送成功。");
		} catch (Exception e) {
			System.out.println("广州用户组告警指标数据推送失败;" + e.getMessage());
			e.printStackTrace();
			System.exit(101);
		} finally {
