...
/**
 * @describe 应用APP告警指标推送入口。
 * @date 2017-11-15
 * @author cym
 *
 */
public class AppQuotaMain {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String fileName = Config.getInstance().getAppQuotaPrefix() + "_" + start_time + ".csv";

			System.out.println("args is [" + start_time + "][" + end_time + "][" + path + "][" + fileName + "]");

			String read_sql = SqlUtils_ReadOracle.getReadAppQuotafromOracle(start_time, end_time);

			List<StringBuffer> StringBufferList = ReadOracleDao.readAppQuota(read_sql, start_time);
			...
			System.out.println("[" + start_time + "]应用告警指标推送成功。");
		} catch (Exception e) {
			System.out.println("广州应用告警指标数据推送失败;" + e.getMessage());
			e.printStackTrace();
			System.exit(101);
		} finally {
